...
| `reason: "budget_insufficiente"` con `totalMs: 0` | il vincolo in §1 | il sito **non è stato contattato**: non c'era tempo nemmeno per iniziare. È l'unico caso in cui il problema è solo di configurazione — `SCRAPE_REQUEST_BUDGET_MS` meno il preambolo (autenticazione, DNS, lettura della ricetta) deve restare sopra `SCRAPE_BROWSER_MIN_MS`, e con i default il margine è **un secondo solo** |
